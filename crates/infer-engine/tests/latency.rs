...
use infer_engine::{
    cpu_placement, cuda_placement, greedy_generate, load_verified_micro, measure_micro_latency,
    micro_kv_layout, reference_engine_build, reference_kernel_bundle, verify_micro_latency,
    write_latency_report, write_synthetic_model, ArchitectureAdapter, ExecutableModel,
    LatencyObservation, MicroAdapter, ReferenceF32Backend, SingleBlockKv, FIXTURE_CASES,
    MAX_CONTEXT, VOCAB,
};
...
fn engine_root() -> infer_contracts::DigestHex {
    ...
    let build = reference_engine_build(
        sha256_prefixed(b"knolo-infer-latency"),
        bundle.root().unwrap(),
    )
    .unwrap();
    build.root().unwrap()
...
fn scratch(name: &str) -> PathBuf {
    let dir =
        std::env::temp_dir().join(format!("knolo-infer-latency-{}-{name}", std::process::id()));
...
#[test]
fn the_micro_fixture_records_time_to_first_token_and_percentiles() {
    ...
    for fixture in FIXTURE_CASES {
        ...
        let measured = measure_micro_latency(&plan, &observed).unwrap();
        ...
        assert_eq!(measured.report.time_to_first_token_nanos, 1_000_000_000);
        assert_eq!(
            measured.report.time_per_output_token_nanos,
            1_000_000_000 / u64::from(fixture.new_tokens)
        );
        assert_eq!(measured.report.request_latency_nanos, 2_000_000_000);
        assert_eq!(measured.report.latency_p50_nanos, 2_000_000_000);
        assert_eq!(measured.report.latency_p95_nanos, 2_000_000_000);
        assert_eq!(measured.report.latency_p99_nanos, 2_000_000_000);
        verify_micro_latency(&measured).unwrap();

        let mut pinned = observed.clone();
        pinned.execution_mode = "pinned".into();
        let pinned = measure_micro_latency(&plan, &pinned).unwrap();
        assert_eq!(
        ...
        let on_slot = measure_micro_latency(&cuda, &observed).unwrap();
        ...
        assert_eq!(
            on_slot.report.latency_p99_nanos,
            measured.report.latency_p99_nanos
        );
    }
    let _ = fs::remove_dir_all(&dir);
}
...
fn a_failed_measurement_issues_no_report() {
    ...
    let err = measure_micro_latency(&plan, &mode).unwrap_err();
    ...
    let err = measure_micro_latency(&plan, &zero_prefill).unwrap_err();
    assert!(err.message.contains("prefill token count is zero"), "{err}");

    let mut mismatch = root.clone();
    mismatch.prefill_tokens = 2;
    let err = measure_micro_latency(&plan, &mismatch).unwrap_err();
    assert!(
    ...
    let err = measure_micro_latency(&plan, &over).unwrap_err();
    ...
    let err = measure_micro_latency(&plan, &vocab).unwrap_err();
    assert!(
    ...
    let err = measure_micro_latency(&plan, &zero_time).unwrap_err();
    ...
    let err = measure_micro_latency(&plan, &overflow).unwrap_err();
    assert!(err.message.contains("latency duration overflows"), "{err}");

    let mut graphs = plan.clone();
    graphs.graph_capture_mode = "decode-buckets".into();
    let err = measure_micro_latency(&graphs, &root).unwrap_err();
    ...
    let err = measure_micro_latency(&rejected, &root).unwrap_err();
    ...
    let err = measure_micro_latency(&wide_context, &root).unwrap_err();
    assert!(
        err.message.contains("latency report is the micro fixture"),
        "{err}"
    );

    let floor = observe(&source, &[1], &[11, 11], 3, 5);
    let floor = measure_micro_latency(&plan, &floor).unwrap();
    assert_eq!(floor.report.time_to_first_token_nanos, 3);
    assert_eq!(floor.report.time_per_output_token_nanos, 2);
    assert_eq!(floor.report.request_latency_nanos, 8);
    assert_eq!(floor.report.latency_p50_nanos, 8);
    assert_eq!(floor.report.latency_p95_nanos, 8);
    assert_eq!(floor.report.latency_p99_nanos, 8);

    let zero_quotient = observe(&source, &[1], &[11, 11], 3, 1);
    let zero_quotient = measure_micro_latency(&plan, &zero_quotient).unwrap();
    assert_eq!(zero_quotient.report.time_per_output_token_nanos, 0);
    assert_eq!(zero_quotient.report.request_latency_nanos, 4);
    assert_eq!(zero_quotient.report.latency_p99_nanos, 4);
    let _ = fs::remove_dir_all(&dir);
}
...
fn the_report_is_written_once_and_a_symlink_is_not_followed() {
    ...
    let measured = measure_micro_latency(&plan, &observed).unwrap();
    write_latency_report(&dir, &measured, "report.cbor").unwrap();
    ...
    let again = write_latency_report(&dir, &measured, "report.cbor").unwrap_err();
    assert!(again.message.contains("already exists"), "{again}");
    assert_eq!(fs::read(dir.join("report.cbor")).unwrap(), stored);

    let outside_name = format!("knolo-latency-not-created-{}", std::process::id());
    let outside = std::env::temp_dir().join(&outside_name);
    let _ = fs::remove_file(&outside);
    std::os::unix::fs::symlink(std::env::temp_dir(), dir.join("escape")).unwrap();
    let err = write_latency_report(&dir, &measured, &format!("escape/{outside_name}")).unwrap_err();
    assert!(err.message.contains("leaves the directory"), "{err}");
    assert!(!outside.exists());

    std::os::unix::fs::symlink(&outside, dir.join("linked.cbor")).unwrap();
    let linked = write_latency_report(&dir, &measured, "linked.cbor").unwrap_err();
    assert!(linked.message.contains("already exists"), "{linked}");
    assert!(!outside.exists());

    let missing = scratch("missing");
    fs::remove_dir_all(&missing).unwrap();
    let err = write_latency_report(&missing, &measured, "report.cbor").unwrap_err();
    assert!(err.message.contains("does not exist"), "{err}");

    let mut changed = measured.clone();
    changed.report.prefill_nanos = 2;
    let err = verify_micro_latency(&changed).unwrap_err();
