//! Prefill, decode, and request throughput for one cold micro-fixture run.
//!
...
//! and `measure_placement_memory` do not call this path. The layout is
//! specified in `spec/KIP-INFER-0028-throughput-report.md`.
...
use infer_contracts::{
    fail, output_token_root, prompt_token_root, tokens_per_second_micros, DigestHex, ErrorCode,
    InferFailure, PlacementPlanV1, ThroughputReportV1,
};
...
/// One micro-fixture observation and the report that records its throughput.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MicroThroughput {
    pub plan: PlacementPlanV1,
    pub observed: ThroughputObservation,
    pub report: ThroughputReportV1,
}

/// Record prefill, decode, and request throughput. No file is created.
pub fn measure_micro_throughput(
    plan: &PlacementPlanV1,
    observed: &ThroughputObservation,
) -> Result<MicroThroughput, InferFailure> {
    let produced = assemble(plan, observed)?;
    verify_micro_throughput(&produced)?;
    Ok(produced)
}

/// Recompute the report from the stored plan and observation.
pub fn verify_micro_throughput(measurement: &MicroThroughput) -> Result<(), InferFailure> {
    ...
    same_u32(
        "throughput concurrency does not match",
        report.concurrency,
        measurement.observed.concurrency,
    )?;
    same_u32(
        "throughput run count does not match",
        report.run_count,
        measurement.observed.run_count,
    )?;
    same_text(
        "throughput warm state does not match",
        &report.warm_state,
        &measurement.observed.warm_state,
    )?;
    same_u32(
        "throughput request count does not match",
    ...
    if recomputed.report.to_bytes()? != report.to_bytes()? {
        return Err(fail(
            ...
            "throughput validation did not match",
        ));
    }
    Ok(())
}
...
pub fn write_throughput_report(
    base: &Path,
    measurement: &MicroThroughput,
    receipt_path: &str,
) -> Result<(), InferFailure> {
    verify_micro_throughput(measurement)?;
...
fn assemble(
    ...
    if plan.graph_capture_mode != "off" {
        return Err(fail(
            ErrorCode::ContractInvalid,
            "graph capture is off for the throughput report",
        ));
    }
        ...
        || plan.kv_precision != "f32"
    {
        return Err(fail(
            ErrorCode::ContractInvalid,
            "throughput report is the micro fixture",
        ));
    }
    ...
    let total = observed
        .prefill_nanos
        .checked_add(observed.decode_nanos)
    ...
    let report = ThroughputReportV1 {
        ...
        prefill_tokens_per_second_micros: tokens_per_second_micros(
            u64::from(observed.prefill_tokens),
            observed.prefill_nanos,
        )?,
        decode_tokens_per_second_micros: tokens_per_second_micros(
            u64::from(observed.decode_tokens),
            observed.decode_nanos,
        )?,
        requests_per_second_micros: tokens_per_second_micros(
            u64::from(observed.request_count),
            total,
        )?,
...
fn in_vocab(tokens: &[u32]) -> Result<(), InferFailure> {
    if tokens.iter().any(|id| *id as usize >= crate::VOCAB) {
        Err(fail(
            ErrorCode::ContractInvalid,
            "throughput token is outside the micro vocabulary",
        ))
    } else {
        Ok(())
    }
...
fn output_path(base: &Path, relative: &str) -> Result<PathBuf, InferFailure> {
    ...
    let base = fs::canonicalize(base).map_err(|err| {
        if err.kind() == std::io::ErrorKind::NotFound {
            fail(
                ErrorCode::ContractInvalid,
                "throughput directory does not exist",
            )
        } else {
            fail(
                ErrorCode::ContractInvalid,
                format!("throughput directory: {err}"),
            )
        }
    })?;
    if !base.is_dir() {
    ...
    for segment in parts {
        ...
        if meta.file_type().is_symlink() || !meta.is_dir() {
            return Err(fail(
                ErrorCode::ContractInvalid,
                "throughput path leaves the directory",
            ));
        }
    }
    ...
    match fs::symlink_metadata(&path) {
        Ok(_) => Err(fail(
            ErrorCode::ContractInvalid,
            "throughput output already exists",
        )),
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => Ok(path),
        Err(err) => Err(fail(
            ErrorCode::ContractInvalid,
            format!("throughput output: {err}"),
        )),
    }
}
